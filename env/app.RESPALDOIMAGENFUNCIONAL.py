...
from os import remove #Permite remover archivos
from os import path #obtiene la ruta del directorio
...
#FOLDER DE IMAGENES QUE FUNCIONA CON LO QUE SE SUBE A TRAVES DE FORMULARIOS DE SUBIDA.
# SI SE CAMBIA LA RUTA avatars TAMBIEN SE DEBE CAMBIAR EN LAS VISTAS HTML
# ESTE UPLOAD TRABAJA CON ADD POST
...
def posts(id):
	date = datetime.now(timezone('America/Chicago'))
	post = Posts.query.get_or_404(id)
	return render_template("posts.html", post=post, date=date)
...
def fotos():
	date 	= 	datetime.now(timezone('America/Chicago'))
	return render_template('fotos.html',date=date, list_Photos = listaArchivos())

# RUTA PARA AGREGAR LAS IMAGENES
@app.route('/add-Fotos', methods=['GET', 'POST'])
def add_Fotos():
	date 	= 	datetime.now(timezone('America/Chicago'))
	if request.method == 'POST':
		if(request.files['archivo']):
	       ...
	       #capturando extensión del archivo ejemplo: (.png, .jpg, .pdf ...etc)
			...
			upload_path = path.join (basepath, 'static/img', nuevoNombreFile) 
			file.save(upload_path)
	return render_template('add_Fotos.html', date=date, list_Photos = listaArchivos())
...
def EliminarFoto(nombreFoto=''):
	...
	if request.method == 'GET':
		...
		url_File = path.join (basepath, 'static/img', nombreFoto)
		#print(url_File)
...
# EN HTML FOTOS.html pegar este código
	# ----------------------------------------------------------------------------
	# <div class="container">
	# 		<div class="row">
	# 		  {% set path = 'static/img/' %}
	...
	# 		       	<img class="img-fluid rounded" src="{{ path }}{{ foto }}"  alt="{{ foto }}">
	# 		        <!-- {{loop.index}} -->
	# 		        <!-- {{ foto }} -->
	...
	# 				<div class="container">
	# 				<div class="btn">
	# 					<a href="{{ url_for('EliminarFoto', nombreFoto = foto ) }}" type="button" class="btn btn-danger" style="margin-top: 40px;">Eliminar Archivo</a>					</div>
	# 				</div>
	# 			{%endif%}
...
# EN HTML PERO EN ADD_FOTOS.HTML AGREGAR ESTE código
 	# ----------------------------------------------------------------------------
	# <form class="form-signin" action="{{ url_for('fotos') }}" method="POST" name="form" enctype="multipart/form-data">
